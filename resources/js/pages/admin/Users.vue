        ...
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-muted-foreground">Gérez les rôles, bannissements et suppressions</p>
                </div>
            </div>
                    ...
                    <TableBody>
                        <TableRow v-for="user in users" :key="user.id">
                            ...
                            <TableCell class="text-center">
                                <Select :model-value="user.roles[0]?.name || 'user'" @update:model-value="(value) => handleRoleChange(user, value)" :disabled="user.is_super_admin">
                                    ...
                                    <SelectContent>
                                        <SelectItem v-for="role in roles" :key="role.id" :value="role.name">
                                            <div class="flex items-center gap-2">
                                                <Shield v-if="role.name === 'admin'" class="h-3 w-3" />
                                                {{ role.name }}
                                            </div>
                                        </SelectItem>
                                    </SelectContent>
                            ...
                            <TableCell class="text-center">
                                <Badge variant="secondary">{{ user.pools_count }}</Badge>
                            </TableCell>
                            <TableCell class="text-center">
                            ...
                            <TableCell class="text-right">
                                <div class="flex justify-end gap-2">
                                    <Button variant="ghost" size="sm" @click="toggleBan(user)" :title="user.is_banned ? 'Débannir' : 'Bannir'" :disabled="user.is_super_admin">
                                        <Ban class="h-4 w-4" />
                                    </Button>
                                    <Button variant="ghost" size="sm" @click="handleDelete(user)" :disabled="user.is_super_admin">
